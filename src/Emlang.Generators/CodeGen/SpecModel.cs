...
/// <summary>
/// One c:/e:/x:/v:/s: element lifted from an emlang spec's slice steps. Lane is the prefix
/// before the first slash ("Game" for events, "Todo" for todo views, "" for bare commands,
/// exceptions and states). 'v' and 's' elements are inert for record emission (SurfaceEmitter
/// filters by kind); they type the `tests:` fixture values for TestsEmitter.
/// </summary>
...
public static class SpecModel
{
    ...
    private static SpecElement? ToElement(YamlMappingNode step)
    {
        foreach (var child in step.Children)
        {
            ...
            return new SpecElement(kind, BareName(raw), Props(step), (int)child.Key.Start.Line, Lane(raw, kind));
        }
        return null;
    }

    /// <summary>Rule (RFC emlang-0003): the prefix before the FIRST slash is the lane ("Game /", "Todo /", …);
    /// a state's swimlane carries no meaning (RFC emlang-0002).</summary>
    private static string Lane(string value, char kind)
    {
        var slash = value.IndexOf('/');
        return kind == 's' || slash < 0 ? string.Empty : value.Substring(0, slash).Trim();
    }

    /// <summary>Rule: events carry the stream prefix ("Game / LobbyOpened"); strip it.</summary>
    private static string BareName(string value) =>
        value.Substring(value.IndexOf('/') + 1).Trim(); // IndexOf -1 when no prefix ⇒ whole string
