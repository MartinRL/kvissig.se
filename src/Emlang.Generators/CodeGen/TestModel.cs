...
/// <summary>One given/when/then step: kind c|e|x|v|s, lane ("" for bare c:/x:/s:), bare name.</summary>
...
public static class TestModel
{
    ...
    private static TestStep ToStep(YamlMappingNode step)
    {
        foreach (var child in step.Children)
        {
            ...
            var slash = raw.IndexOf('/'); // first slash: RFC emlang-0003
            return new TestStep(
                kind,
                kind == 's' || slash < 0 ? string.Empty : raw.Substring(0, slash).Trim(), // a state's swimlane carries no meaning
                raw.Substring(slash + 1).Trim(),
                Props(step),
                (int)child.Key.Start.Line);
        }
        throw new InvalidDataException($"test step at line {step.Start.Line} has no c/e/x/v/s key");
    }
