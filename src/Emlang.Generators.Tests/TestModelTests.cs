...
namespace Emlang.Generators.Tests;
...
public class TestModelTests
{
    ...
    [Fact]
    public void Tank_fixture_parses_the_full_census_of_43_tests()
    {
        var tests = TankTests();

        tests.Should().HaveCount(43);
        tests.Count(t => t.When is not null).Should().Be(28, "GWT decide cases");
        tests.Count(t => t.When is null && t.Then is [{ Kind: 's' }])
            .Should().Be(5, "GT fold cases (2 + the 3 dialect folds)");
