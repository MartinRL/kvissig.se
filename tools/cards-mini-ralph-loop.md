# Ralph loop — three 175-card mini decks

Feed this to `/ralph-loop`. Builds three **mini (175-card, 7-question)** decks via the
question pipeline, SEQUENTIALLY (hund → elbil → fotboll). Terminates deterministically
when ALL three are merged at exactly 175 cards and `dotnet test` is green.

---

Goal: three mini packs in `src/MerEllerMindre.Domain/data/packs/`:
...
| `fotboll` | `fotboll-mini.csv`   | `15,40,30,15` | Allsvenskan-klubbar + spelare; marknadsvärde mkr (pinna säsong+källa Transfermarkt+år), grundadår-som-ålder, mål/säsong, publiksnitt/säsong, SM-guld |
...
- The loop authors from REAL entities/figures only — it never invents facts or entities.

## Each iteration (work the themes SEQUENTIALLY hund → elbil → fotboll — finish one to 175 +
merge before starting the next; the budget brake beats interleaving)
...
card, ~27/batch). Staging = state: a limit hit mid-run is resumable, no rework.
...
5. `dotnet run tools/pack.cs -- report --staging --dir question-staging/<tema> --key question`
...
## Stop
Write `CARDS-DONE` when all three packs are merged at exactly 175 cards AND `dotnet test` is
green.
...
- Pool runs short of distinct entities for a band/theme → STOP and report the gap (never invent
  entities or facts).
- On usage-limit interruption: the loop stops; restart it — it resumes from staging.
