...
source: "https://www.marcusoft.net/2025/02/getting-started-with-htmx-dotnet.html"
author:
  - "[[Marcus Hammarberg]]"
published: 2025-02-21
created: 2026-06-13
description: "HTMX is great! And I really like .NET Minimal APIs. Let’s put it together and see how it works. When I first thought about creating a backend for HTMX using .NET I went down the rabbit hole of Razor Pages. Which was not great I have to say. That programming model is weird and I don’t like it. But in my spelunking I found Razor Slices which will work great as a template engine. That means that we have a lightweight and easy to grasp backend with Minimal API and then use Razor Slices to render the HTML."
tags:
  - "clippings"
---
...
```html
Show code@inherits RazorSliceHttpResult
...
2. It will trigger a HTTP GET to `/todos/create`
3. It will take the response and replace (swap) all the content of the element with the response.
4. It will swap the element of `this` element, since we didn’t specify a `hx-target`.
...
2. The the favicon.ico and the site.css.
3. HTMX will also be loaded (and most likely cached)

But then you’ll see an entry called `create` that is a HTTP GET to \` [http://localhost:xxxx/todos/create](http://localhost:xxxx/todos/create)`. This request was issued by HTMX when the ` <div id="todo-form">\` was loaded.
...
2. Target the `#todo-list` our main list of todos.
3. Swap in the response before the end of the list, appending items
...
In order to do the server-side validation we need to do the validation and *change* what is returned from the route. Here’s how the form tag looks now:
